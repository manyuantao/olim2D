...
/*--------- global variables ---------*/
char chfield = 'd'; // choose the vector field b(x)
char ch_exact_sol; // indicates whether the exact solution is available or not
const double fac = 10.0; // fac = the ratio of the magnitudes of the rotational and the potential components
const long nx1 = NX - 1, ny1 = NY - 1, NXY = NX*NY, nx2 = NX - 2, ny2 = NY - 2;
const long KK = K*K;
double hx,hy,hmax,dmaxsquared;
char ms[NX*NY]; // mesh point status: 0 = 'Unknown', 1 = 'Considered', 2 = "in Accepted Front", 3 = "Accepted"
double Uexact[NX*NY], g[NX*NY]; // function to be computed
...
void param() {
    ...
    printf("x_ipoint: Iindex = %li, %.4e, %.4e\n",Iindex,x_ipoint.x,x_ipoint.y);
...
int main() {
    long i,j,k,ind,NAC;
    double cpu, dd, errmax = 0.0, erms = 0.0, umax = 0.0, urms = 0.0;
    clock_t CPUbegin;
    double q1,q2,qc2;
    
    if( CH_SHOOT_MAP == 'y' || CH_SHOOT_MAP == 'n' ) {
        printf("chfield = %c, fac = %g\n",chfield,fac);
        N1ptu = 0;
        N2ptu = 0;
        N2call = 0;
        param();
